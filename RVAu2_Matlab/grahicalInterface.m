...
threshold = str2double(M2{indexshelf,5});

auto = get(handles.radioButtonAuto, 'Value');
indexMethod = get(handles.popupMetodoProcura,'Value');
if auto == 0 
    % a obter a homografia (Neste caso a escolha dos pontos é manual).
    [image, tform] = homograph_manual(shelfPath, str2double(M2{indexshelf,3}), str2double(M2{indexshelf, 4}));
else
    % a  obter os quatro cantos da estante automaticamente
    input_points = cornerdetection(shelf, threshold);
    % a obter a homografia
    [image, tform] = homograph_auto(shelfPath, str2double(M2{indexshelf,3}), str2double(M2{indexshelf, 4}), input_points);
end

if indexMethod == 1
% a detetar os quatro cantos do livro na homografia da prateleira em
% pose frontal
...
elseif  indexMethod == 2 % houghlines
    myhoughLines(image, template);
else % backprojection
	detectedImage = backprojection(image, template);
    axes(handles.shelfImageDisplay); %#ok<MAXES>
    imshow(detectedImage);
end
